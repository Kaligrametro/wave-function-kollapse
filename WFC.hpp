...
public:
	...
	static void export_patterns(std::vector<Pattern> patterns)
	{
		...
		{
			std::vector<bmp::Pixel> pixeles(VOL);
			for (size_t e = 0; e < patterns[i].data.size(); e++)
			{
				pixeles[e] = static_cast<bmp::Pixel>(patterns[i].data[e]);
			}
			file.setPixels(pixeles);
			filename = "output/pattern(" + std::to_string(i) + ").bmp";
			file.save(filename);
		}
	}
#else
